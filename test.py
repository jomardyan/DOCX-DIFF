...
class DocxDiffGUI:
    def __init__(self, root):
        ...
        ttk.Entry(file_frame, textvariable=self.file_a_var, width=80).grid(row=0, column=1, padx=2, sticky="ew")
        ttk.Button(file_frame, text="Browse", command=lambda: self.browse_file('a'), width=8).grid(row=0, column=2, padx=2)
        
        # File B
        ttk.Label(file_frame, text="B:").grid(row=1, column=0, sticky="w", pady=2, padx=(5, 2))
        self.file_b_var = tk.StringVar()
        ttk.Entry(file_frame, textvariable=self.file_b_var, width=80).grid(row=1, column=1, padx=2, sticky="ew")
        ttk.Button(file_frame, text="Browse", command=lambda: self.browse_file('b'), width=8).grid(row=1, column=2, padx=2)
        ...
        ttk.Spinbox(options_frame, from_=0, to=20, textvariable=self.context_var, width=6).grid(row=0, column=1, padx=2)
        
        self.ignore_case_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(options_frame, text="Ignore Case", variable=self.ignore_case_var).grid(row=0, column=2, padx=5)
        
        self.ignore_whitespace_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(options_frame, text="Ignore Space", variable=self.ignore_whitespace_var).grid(row=0, column=3, padx=5)
        
        # View options
        ttk.Label(options_frame, text="Font:").grid(row=0, column=4, sticky="w", padx=(10, 2))
        self.font_size_var = tk.IntVar(value=10)
        ttk.Spinbox(options_frame, from_=8, to=20, textvariable=self.font_size_var, width=6, 
                    command=self.update_font_size).grid(row=0, column=5, padx=2)
        
        self.show_line_numbers_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(options_frame, text="Line #", variable=self.show_line_numbers_var,
                       command=self.toggle_line_numbers).grid(row=0, column=6, padx=5)
        ...
        ttk.Button(button_frame, text="Compare", command=self.compare_files, width=8, 
                  style='Green.TButton').pack(side="left", padx=2)
        ttk.Button(button_frame, text="Clear", command=self.clear_results, width=6).pack(side="left", padx=2)
        ttk.Button(button_frame, text="Export TXT", command=self.export_txt, width=9).pack(side="left", padx=2)
        ttk.Button(button_frame, text="Export HTML", command=self.export_html, width=10).pack(side="left", padx=2)
        ttk.Button(button_frame, text="Search", command=self.show_search_dialog, width=7).pack(side="left", padx=2)
        
        ttk.Separator(button_frame, orient=tk.VERTICAL).pack(side="left", fill="y", padx=5)
        
        ttk.Button(button_frame, text="◄", command=self.prev_diff, width=3).pack(side="left", padx=1)
        ttk.Button(button_frame, text="►", command=self.next_diff, width=3).pack(side="left", padx=1)
        
        self.filter_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(button_frame, text="Changes Only", variable=self.filter_var).pack(side="left", padx=5)
        ...
        ttk.Button(unified_toolbar, text="+", command=lambda: self.adjust_font(1), width=3).pack(side="left", padx=1)
        ttk.Button(unified_toolbar, text="-", command=lambda: self.adjust_font(-1), width=3).pack(side="left", padx=1)
        ttk.Separator(unified_toolbar, orient=tk.VERTICAL).pack(side="left", fill="y", padx=3)
        ttk.Button(unified_toolbar, text="Copy All", command=self.copy_all_diff, width=8).pack(side="left", padx=2)
        ttk.Button(unified_toolbar, text="Copy Selection", command=self.copy_selection, width=12).pack(side="left", padx=2)
        ...
        h_scrollbar = ttk.Scrollbar(unified_frame, orient=tk.HORIZONTAL, command=self.results_text.xview)
        ...
        ttk.Button(sbs_toolbar, text="+", command=lambda: self.adjust_font(1), width=3).pack(side="left", padx=1)
        ttk.Button(sbs_toolbar, text="-", command=lambda: self.adjust_font(-1), width=3).pack(side="left", padx=1)
        ttk.Separator(sbs_toolbar, orient=tk.VERTICAL).pack(side="left", fill="y", padx=3)
        
        self.sync_scroll_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(sbs_toolbar, text="Sync Scroll", variable=self.sync_scroll_var).pack(side="left", padx=5)
        
        ttk.Button(sbs_toolbar, text="Copy Left", command=lambda: self.copy_pane('left'), width=9).pack(side="left", padx=2)
        ttk.Button(sbs_toolbar, text="Copy Right", command=lambda: self.copy_pane('right'), width=9).pack(side="left", padx=2)
        ...
        ttk.Label(left_frame, text="File A (Original)", font=("Segoe UI", 9, "bold"), background="#ffdce0", 
                 padding=2).pack(fill="x")
        self.left_text = scrolledtext.ScrolledText(left_frame, wrap=tk.NONE, font=("Consolas", 10),
                                                    padx=5, pady=2)
        ...
        ttk.Label(right_frame, text="File B (Modified)", font=("Segoe UI", 9, "bold"), background="#d1f7d6",
                 padding=2).pack(fill="x")
        self.right_text = scrolledtext.ScrolledText(right_frame, wrap=tk.NONE, font=("Consolas", 10),
                                                     padx=5, pady=2)
    ...
    def next_diff(self):
        ...
        self.status_bar.config(text=f"Difference {self.current_diff_index + 1} of {len(self.diff_positions)}")
    ...
    def compare_files(self):
        ...
        try:
            ...
            stats_text = f"Similarity: {similarity_ratio:.2f}% | Additions: {additions} | Deletions: {deletions} | Changes: {changes}"
        ...
        if filename:
            try:
                with open(filename, 'w', encoding='utf-8') as f:
                    f.write(f"DOCX Comparison Report\n")
        ...
        if filename:
            try:
                with open(filename, 'w', encoding='utf-8') as f:
                    ...
                    f.write(".stats { background-color: #f0f0f0; padding: 10px; margin-bottom: 20px; border-radius: 5px; }\n")
                    ...
                    f.write(f"<p><strong>Similarity:</strong> {self.stats['similarity']:.2f}%</p>\n")
